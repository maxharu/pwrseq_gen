...
或直接雙擊 `build.bat`（需已安裝 Python 與上述套件）。

打包完成後，`dist\PowerSeqGen` 資料夾內會產生 `PowerSeqGen.exe`。**請將整個 `dist\PowerSeqGen` 資料夾**複製到目標電腦即可執行，無需安裝 Python。
