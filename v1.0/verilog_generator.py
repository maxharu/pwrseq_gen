...
def generate_verilog(config: PowerSeqConfig, output_filename: str | None = None) -> str:
    ...
    """
    rails = _topological_sort(config.rails)
    name_to_rail = {r.name: r for r in rails}
    successors = _get_successors(config)
    sequenced = [r for r in rails if r.has_pseqcell]
    externals = [r for r in rails if r.seq_type == "input"]
    ...
    inputs_with_deb = [r for r in externals if getattr(r, "deb_enable", False)]
